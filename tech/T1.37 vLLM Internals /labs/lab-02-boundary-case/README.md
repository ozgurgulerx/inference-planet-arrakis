# T1.37-L02 — Feature/failure compatibility

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; selected feature support conditional · Dependencies: T1.37-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

What breaks when prefix caching, CUDA graphs or an advanced connector changes the execution path?
...
1. Create `results/T1.37-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Select one supported feature, capture baseline/feature traces, then exercise one bounded failure or fallback.
...
- Independent conditions: feature off/on plus documented boundary case.
...
- Measure: component path, kernel/graph behavior, correctness and fallback.
...
Falsifier: **A CLI flag is treated as proof the feature executed.**
