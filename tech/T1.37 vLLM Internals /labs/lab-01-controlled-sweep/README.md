# T1.37-L01 — Scheduler and block-manager pressure

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.37-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How do token budget and KV pressure change scheduling, preemption and latency?
...
1. Create `results/T1.37-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay a fixed mixed trace while changing one supported scheduler limit.
...
- Independent conditions: baseline and one token/sequence limit; below/near capacity.
...
- Measure: scheduled tokens, blocks, preemption, TTFT/TPOT and goodput.
...
Falsifier: **The workload or model length changes with the scheduler control.**
