# T1.24-L01 — Latency/bandwidth size curve

Status: planned, not executed · Scope: core · Level: medium · Hardware: N2 RDMA required · Dependencies: T1.24-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How do one-sided write/read and two-sided send behave across sizes?
...
1. Create `results/T1.24-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run versioned perftest tools with fixed queue depth and repetitions on authorized RDMA endpoints.
...
- Independent conditions: operation type × geometric payload × queue depth.
...
- Measure: latency, bandwidth, completions, retries/errors and CPU.
...
Falsifier: **TCP fallback or loopback is presented as fabric RDMA.**
