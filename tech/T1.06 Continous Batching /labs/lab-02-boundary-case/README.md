# T1.06-L02 — Preemption and overload

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 · Dependencies: T1.06-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

What failure mode appears first beyond the sustainable continuous-batching knee?
...
1. Create `results/T1.06-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Increase offered load through saturation with hard request/time/inflight bounds, then test one admission control.
...
- Independent conditions: open-loop rates below/at/above knee; control off/on.
...
- Measure: queue depth, preemption, rejection, goodput and recovery time.
...
Falsifier: **The generator cannot maintain offered load or hidden retries erase failures.**
