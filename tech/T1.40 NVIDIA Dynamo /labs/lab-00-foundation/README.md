# T1.40-L00 — Dynamo component graph

Status: planned, not executed · Scope: core · Level: intro · Hardware: G1; container/orchestration support conditional · Dependencies: none

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which frontend, router and worker owns each request and state transition?
...
1. Create `results/T1.40-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Deploy the smallest official version-matched Dynamo recipe possible and capture service discovery plus one request trace.
...
- Independent conditions: one frontend/router and one aggregated worker.
...
- Measure: component topology, registrations, route, output and exact versions.
...
Falsifier: **A diagram is accepted without matching runtime service identities.**
