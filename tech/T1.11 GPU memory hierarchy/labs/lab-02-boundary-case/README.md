# T1.11-L02 — Decode byte attribution

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 profiler permissions conditional · Dependencies: T1.11-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which weight, KV and activation bytes plausibly limit decode at different batches?
...
1. Create `results/T1.11-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Combine a byte ledger with a short Nsight trace and a batch sweep.
...
- Independent conditions: decode batch 1/4/16; fixed context and precision.
...
- Measure: estimated bytes, HBM traffic proxy, arithmetic intensity and token rate.
...
Falsifier: **The claimed HBM limit persists when batch raises reuse without the predicted shift.**
