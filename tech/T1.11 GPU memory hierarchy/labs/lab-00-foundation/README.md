...
What bandwidth is observed for pageable H2D, pinned H2D, D2H and device copies?
...
1. Create `results/T1.11-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Implement a synchronized size sweep with allocation mode and direction recorded.
...
- Independent conditions: 1KiB–1GiB geometric sizes; pageable/pinned/device paths.
...
- Measure: latency, effective bandwidth, allocation time and correctness.
...
Falsifier: **Asynchronous copies are timed without synchronization or allocation cost is silently mixed.**
