# T1.36-L00 — Speculation accounting
...
How do proposal depth, accepted tokens and target verification rounds determine possible speedup?
...
1. Create `results/T1.36-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Build a deterministic simulator or tiny draft/target pair and log every proposed, accepted and rejected token.
...
- Independent conditions: acceptance 0/partial/full across proposal depths.
...
- Measure: target rounds, draft work, acceptance length and exact output.
...
Falsifier: **Accepted-token counts cannot reconstruct the emitted sequence.**
