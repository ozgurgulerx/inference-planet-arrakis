# T1.25-L02 — Lifetime and stale-key failure

Status: planned, not executed · Scope: later · Level: expert · Hardware: N2 RDMA required · Dependencies: T1.25-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How should ownership prevent a key or buffer from being reused after deregistration?
...
1. Create `results/T1.25-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Build an isolated lifetime test with explicit state transitions and hard timeout using owned endpoints.
...
- Independent conditions: valid lifetime, early deregistration and delayed consumer.
...
- Measure: completion/error, cleanup and invariant violations.
...
Falsifier: **The test risks arbitrary remote memory or lacks bounded isolation.**
