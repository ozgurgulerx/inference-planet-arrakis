# T1.23-L01 — Local versus remote host memory

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1; dual-socket exposure conditional · Dependencies: T1.23-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does CPU binding and memory placement change pinned-transfer performance?
...
1. Create `results/T1.23-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Allocate pinned buffers under local and remote NUMA bindings where controllable, then run the same H2D/D2H sweep.
...
- Independent conditions: local/remote CPU+memory bindings; matched GPU.
...
- Measure: transfer bandwidth, CPU time, page placement and variance.
...
Falsifier: **The provider does not expose two NUMA nodes or binding cannot be verified.**
