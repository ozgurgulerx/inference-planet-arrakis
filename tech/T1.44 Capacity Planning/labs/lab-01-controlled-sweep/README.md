# T1.44-L01 — Saturation and goodput curve

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.44-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

What is the sustainable offered-load knee under frozen per-class SLOs?
...
1. Create `results/T1.44-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run open-loop rates through saturation with bounded inflight/time and all failures retained.
...
- Independent conditions: at least four rates around the predicted knee.
...
- Measure: offered/completed rate, queue, per-class tails, errors and SLO goodput.
...
Falsifier: **The load generator self-throttles or failed requests leave the denominator.**
