# T1.02-L02 — Architecture boundary change

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; changed model support conditional · Dependencies: T1.02-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which conclusions survive when the attention/state architecture changes?
...
1. Create `results/T1.02-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Repeat the ledger on one documented changed architecture (GQA, MLA, sliding-window or recurrent hybrid) without reusing dense-MHA assumptions.
...
- Independent conditions: baseline architecture versus one changed architecture at matched effective workload.
...
- Measure: persistent bytes/token, compute path, quality-valid output and unsupported assumptions.
...
Falsifier: **The claimed architectural mechanism disappears after matching model size and actual token counts.**
