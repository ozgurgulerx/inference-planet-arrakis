# T1.35-L01 — Skew and capacity-factor sweep

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.35-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How do token skew and expert capacity affect latency and dropped/rerouted work?
...
1. Create `results/T1.35-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay controlled balanced and biased token batches through the same MoE model/runtime.
...
- Independent conditions: balanced/biased × low/high batch; supported capacity control if exposed.
...
- Measure: max-rank/expert load, dispatch time, drops, TPOT and output validity.
...
Falsifier: **Request counts are used instead of expert-token counts.**
