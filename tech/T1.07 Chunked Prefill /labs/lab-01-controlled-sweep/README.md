# T1.07-L01 — Chunk-size frontier

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.07-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Where does smaller chunking stop helping because launch and scheduling overhead dominate?
...
1. Create `results/T1.07-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Sweep supported chunk/token-budget settings and capture timelines around the interference window.
...
- Independent conditions: at least three chunk budgets from coarse to fine.
...
- Measure: short tail latency, long completion, kernels/step, scheduler CPU and throughput.
...
Falsifier: **No U-shaped or monotonic mechanism survives replication.**
