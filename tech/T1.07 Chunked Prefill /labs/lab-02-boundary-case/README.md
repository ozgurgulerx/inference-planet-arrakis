# T1.07-L02 — Fairness changed case

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 · Dependencies: T1.07-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does the selected chunk size remain defensible under bursty arrivals and longer outputs?
...
1. Create `results/T1.07-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay a burst trace and a decode-heavy trace using the chosen and neighboring settings.
...
- Independent conditions: steady versus bursty; short versus long OSL.
...
- Measure: per-class SLO goodput, starvation, queue and compute utilization.
...
Falsifier: **The chosen setting wins only on the tuning trace.**
