# T1.12-L02 — Held-out bottleneck prediction

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 · Dependencies: T1.12-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Can the roofline model predict a changed shape, including a launch- or communication-limited point outside the simple compute/HBM roofline?
...
1. Create `results/T1.12-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Commit a prediction for a held-out shape, run it, then reconcile the largest error term. Include one tiny-work launch-dominated point; add a collective-dominated point only when G2 topology is verified, otherwise keep communication as a separately labeled analytical ceiling.
...
- Independent conditions: one unseen skinny or long-context shape; one tiny launch-dominated shape; optional G2 communication-dominated point.
...
- Measure: predicted versus observed latency, launch count/gap, optional collective critical-path share and bottleneck class.
...
Falsifier: **The prediction is edited after observation or lacks an error bound.**
