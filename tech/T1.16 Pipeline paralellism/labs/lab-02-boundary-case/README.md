# T1.16-L02 — Interactive-serving boundary

Status: planned, not executed · Scope: later · Level: expert · Hardware: G2 · Dependencies: T1.16-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Why can pipeline parallelism lose at low concurrency despite fitting the model?
...
1. Create `results/T1.16-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Compare latency at low and high request concurrency using the same partition.
...
- Independent conditions: concurrency 1/2 versus enough microbatches to fill pipeline.
...
- Measure: TTFT, E2E, bubble fraction and goodput.
...
Falsifier: **The result changes due to batching policy rather than pipeline occupancy.**
