# T1.32-L01 — Quality/performance/capacity matrix

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.32-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which of memory capacity, bandwidth, compute and quality changes actually explain the result?
...
1. Create `results/T1.32-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run matched prompts, sampling and load for baseline and quantized conditions with frozen quality checks.
...
- Independent conditions: short/long × concurrency 1/8; fixed output ceilings.
...
- Measure: memory, TTFT/TPOT, throughput, power and task validity.
...
Falsifier: **Speed is claimed while failed/changed outputs are excluded from the denominator.**
