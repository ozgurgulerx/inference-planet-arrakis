# T1.38-L00 — SGLang request and scheduler trace
...
How does one request traverse SGLang's server, scheduler and model worker?
...
1. Create `results/T1.38-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Launch a pinned official SGLang release on the baseline model or nearest supported small model and capture one request path.
...
- Independent conditions: OSL 1 and 32; warm server.
...
- Measure: component events, TTFT/TPOT, memory and exact launch.
...
Falsifier: **Results are compared with vLLM before workload/tokenizer parity is checked.**
