# T1.26-L00 — InfiniBand fabric inventory

Status: planned, not executed · Scope: core · Level: intro · Hardware: N2 InfiniBand conditional · Dependencies: none

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which HCA, port, link rate, MTU and fabric path are actually available?
...
1. Create `results/T1.26-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Capture device/port state and provider topology; stop as blocked if no verbs device is exposed.
...
- Independent conditions: each visible HCA/port.
...
- Measure: link layer/state/rate/MTU/GID and permissions.
...
Falsifier: **Ethernet connectivity alone is called InfiniBand.**
