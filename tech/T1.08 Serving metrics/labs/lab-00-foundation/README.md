...
What exact events and populations define TTFT, TPOT, ITL, E2E and throughput?
...
1. Create `results/T1.08-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Generate a synthetic SSE stream with known delays, then one real stream; reconstruct every metric from raw timestamps.
...
- Independent conditions: K=1, K=2, normal K and cancelled request.
...
- Measure: event ledger, formula output, undefined cases and usage reconciliation.
...
Falsifier: **A metric is computable only by treating chunks as tokens or summing percentiles.**
