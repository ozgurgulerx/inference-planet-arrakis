# T1.22-L01 — Peer bandwidth and contention

Status: planned, not executed · Scope: core · Level: medium · Hardware: G2+; NVLink required · Dependencies: T1.22-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does observed pair bandwidth match the exposed scale-up path, and how does concurrent traffic change it?
...
1. Create `results/T1.22-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run directed pair tests alone and concurrently on disjoint/shared paths.
...
- Independent conditions: single pair, disjoint pairs and contending pairs.
...
- Measure: bandwidth matrix, aggregate bandwidth, link counters and fairness.
...
Falsifier: **The topology does not support the assumed disjoint/shared path distinction.**
