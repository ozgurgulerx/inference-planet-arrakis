# T1.20-L02 — Overlap and contention

Status: planned, not executed · Scope: later · Level: expert · Hardware: G2 · Dependencies: T1.20-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When can collective work overlap compute, and when does shared-resource contention erase the gain?
...
1. Create `results/T1.20-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Place controlled compute before/concurrent with collectives on explicit streams and verify dependencies.
...
- Independent conditions: communication only, compute only, overlapped; light/heavy compute.
...
- Measure: critical-path time, overlap, SM/link utilization and correctness.
...
Falsifier: **Summed component times are used instead of the observed critical path.**
