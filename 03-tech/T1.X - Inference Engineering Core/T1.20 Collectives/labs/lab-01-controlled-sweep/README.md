# T1.20-L01 — Latency-bandwidth sweep

Status: planned, not executed · Scope: core · Level: medium · Hardware: G2 · Dependencies: T1.20-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How do collective type and payload size change the critical path?
...
1. Create `results/T1.20-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Benchmark the four collectives over the same geometric size grid and topology.
...
- Independent conditions: tiny through large messages; fixed ranks and dtype.
...
- Measure: latency, effective bandwidth, per-rank time and link utilization.
...
Falsifier: **Different byte definitions make the curves incomparable.**
