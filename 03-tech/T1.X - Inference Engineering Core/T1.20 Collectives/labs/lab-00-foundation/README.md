# T1.20-L00 — Collective semantics
...
What data does each rank own before and after all-reduce, all-gather, reduce-scatter and all-to-all?
...
1. Create `results/T1.20-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Implement two-rank tensor examples with unique rank values and assert exact outputs.
...
- Independent conditions: four collectives at tiny sizes.
...
- Measure: input/output ledger, checksums and payload formula.
...
Falsifier: **Any collective is explained only by analogy without exact rank-local output.**
