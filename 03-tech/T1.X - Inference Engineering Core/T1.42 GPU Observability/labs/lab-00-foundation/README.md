# T1.42-L00 — GPU telemetry baseline

Status: planned, not executed · Scope: core · Level: intro · Hardware: G1; DCGM conditional · Dependencies: none

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which clocks, power, temperature, memory and engine metrics are actually observable on this Pod?
...
1. Create `results/T1.42-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Inventory `nvidia-smi`, DCGM/exporter availability, field semantics and permissions; preserve missing signals.
...
- Independent conditions: idle and one known compute workload.
...
- Measure: available fields, units, cadence, labels and gaps.
...
Falsifier: **A configured metric is assumed emitted/supported without `/metrics` evidence.**
