# T1.42-L01 — Request-correlated dashboard

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.42-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Can telemetry changes be aligned with bounded request phases without mixing clocks?
...
1. Create `results/T1.42-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run a deterministic phase-marked workload while sampling GPU telemetry and engine metrics.
...
- Independent conditions: prefill-heavy, decode-heavy and idle intervals.
...
- Measure: correlated windows for SM, memory, PCIe/NVLink, clocks, power and request metrics.
...
Falsifier: **Timestamp alignment is finer than scrape cadence or crosses unsynchronized clocks.**
