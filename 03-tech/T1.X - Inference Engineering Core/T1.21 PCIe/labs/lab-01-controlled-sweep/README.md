# T1.21-L01 — Pinned versus pageable transfer

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.21-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

At what size does pinned-memory setup repay faster H2D/D2H transfer?
...
1. Create `results/T1.21-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Benchmark allocation plus transfer and reused-buffer transfer separately over a size sweep.
...
- Independent conditions: pageable/pinned × allocate-each/reuse × H2D/D2H.
...
- Measure: setup time, transfer latency, effective bandwidth and checksum.
...
Falsifier: **Registration/allocation cost is hidden from the claimed use case.**
