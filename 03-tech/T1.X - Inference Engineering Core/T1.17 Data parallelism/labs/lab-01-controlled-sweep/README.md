# T1.17-L01 — DP1 versus DP2 serving

Status: planned, not executed · Scope: core · Level: medium · Hardware: G2 · Dependencies: T1.17-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When does a second replica improve SLO goodput more than TP2?
...
1. Create `results/T1.17-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run one then two independent one-GPU servers behind a bounded router using matched offered load.
...
- Independent conditions: DP1/DP2; round-robin and queue-aware; same trace.
...
- Measure: per-replica queue, TTFT, throughput, failures and cost.
...
Falsifier: **Requests silently retry or model state differs across replicas.**
