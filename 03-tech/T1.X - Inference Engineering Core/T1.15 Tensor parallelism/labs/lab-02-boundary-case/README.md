# T1.15-L02 — Topology sensitivity

Status: planned, not executed · Scope: later · Level: expert · Hardware: G2; alternate topology conditional · Dependencies: T1.15-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Would the TP conclusion change across a slower peer path or larger batch?
...
1. Create `results/T1.15-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Use the observed P2P matrix to predict a changed topology/batch; test only where controllable.
...
- Independent conditions: fast pair versus slower pair, or measured simulation when no alternative pair.
...
- Measure: collective time share, token rate and prediction error.
...
Falsifier: **A simulated link is presented as hardware proof.**
