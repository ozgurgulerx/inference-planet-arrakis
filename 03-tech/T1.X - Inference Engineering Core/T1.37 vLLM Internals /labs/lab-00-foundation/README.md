# T1.37-L00 — vLLM request autopsy
...
Which vLLM components own a request from API parse through scheduler, KV manager, model runner and output?
...
1. Create `results/T1.37-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Trace one request with IDs and source anchors in the pinned vLLM release; record queue, scheduling, block and execution events.
...
- Independent conditions: one request, OSL 1 then 16.
...
- Measure: component/event ledger, source anchors and state release.
...
Falsifier: **Source from a different release is mixed with runtime observations.**
