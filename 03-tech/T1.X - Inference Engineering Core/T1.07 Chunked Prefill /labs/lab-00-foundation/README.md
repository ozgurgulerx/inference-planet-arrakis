# T1.07-L00 — Long-prompt interference
...
Does chunking protect active decodes from a newly arrived long prefill?
...
1. Create `results/T1.07-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay the same long-plus-short trace with chunked prefill disabled and enabled.
...
- Independent conditions: one long prompt injected during short decode traffic; matched token budget.
...
- Measure: short ITL/TPOT tail, long TTFT, iteration occupancy and goodput.
...
Falsifier: **The run changes more than chunking or actual token counts are unmatched.**
