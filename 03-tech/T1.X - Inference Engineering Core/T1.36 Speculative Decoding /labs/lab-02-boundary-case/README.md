# T1.36-L02 — Acceptance-collapse boundary

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 · Dependencies: T1.36-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which prompt/domain or load makes speculation lose?
...
1. Create `results/T1.36-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Choose a changed workload predicted to lower acceptance and sweep proposal depth if supported.
...
- Independent conditions: in-domain versus changed domain; low/high load.
...
- Measure: acceptance, verification waste, latency and goodput.
...
Falsifier: **The changed case also changes target model or output policy.**
