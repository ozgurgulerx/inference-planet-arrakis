...
| expert fields | total experts, experts selected per token, shared experts and resident versus active weights | What must fit, execute and cross the network? |
...
### Full, local and sparse attention change which positions are read

- **Full causal attention** lets each query read every retained position up to and including its own position.
- **Local or sliding-window attention** reads a bounded neighborhood, usually the most recent \(W\) positions. It reduces persistent state only when the architecture and runtime are allowed to evict older K/V; global, sink or periodic full-attention layers can preserve a growing term.
- **Sparse attention** reads a selected subset of history. The pattern may be fixed, block-structured or data-dependent. Reading \(K\ll S\) positions does not by itself prove that only \(K\) positions are stored or that selection is cheap.

This position-selection axis is separate from the K/V-representation axis. A local layer can use MHA or GQA; MLA compresses the cached representation rather than defining which positions are visible. Keep both axes in the state ledger.
...
### Sinusoidal encodings and RoPE put position in different places

The original Transformer added a fixed sinusoidal vector to each token embedding:

\[
PE_{p,2i}=\sin\left(p/10000^{2i/H}\right),
\qquad
PE_{p,2i+1}=\cos\left(p/10000^{2i/H}\right).
\]

That changes the residual input before the first block. RoPE instead rotates pairs of Q and K coordinates inside attention. Its useful property is:

\[
(R_pq)^\top(R_kk)=q^\top R_{k-p}k,
\]

so a query-key dot product depends naturally on relative displacement. V is not rotated. Sinusoidal addition and RoPE are trained architectural choices, not interchangeable runtime flags. Changing a configuration's maximum position alone does not validate longer context: positional scaling, training distribution, attention cost and state capacity remain separate constraints. See [Attention Is All You Need](https://arxiv.org/abs/1706.03762) and [RoFormer](https://arxiv.org/abs/2104.09864).

### LayerNorm, RMSNorm, Q/K norm and placement are distinct choices

LayerNorm recenters and rescales each token's hidden vector:

\[
\operatorname{LayerNorm}(x)=
\gamma\odot
\frac{x-\mu}{\sqrt{\frac1H\sum_i(x_i-\mu)^2+\epsilon}}+\beta.
\]

RMSNorm keeps the rescaling but omits mean subtraction:
...
Its FLOP count is small, but during decode a separate reduction and launch repeated twice per layer can matter. Production runtimes commonly fuse residual addition and normalization.

Norm placement changes the residual graph:
...
Pre-norm versus post-norm is especially consequential during training, but inference must still reproduce the learned order exactly; moving a norm changes the model and its fusion/dataflow opportunities. See [On Layer Normalization in the Transformer Architecture](https://arxiv.org/abs/2002.04745).

Q/K norm is a third location: it normalizes projected query and key heads rather than the residual stream. Qwen3 applies Q/K norm before RoPE. It does not replace the block's RMSNorm, and the exact trained operation order is part of model correctness, not an optional optimization.

### ReLU and SwiGLU imply different FFN graphs

The original Transformer used a two-projection dense FFN with a pointwise ReLU:

\[
\operatorname{FFN}_{\text{ReLU}}(x)=\max(0,xW_1+b_1)W_2+b_2.
\]

SwiGLU adds a learned gate and uses SiLU/Swish on one branch:
...
ReLU's elementwise activation is not the main cost; its two matrices are. SwiGLU has gate, up and down matrices, so equal hidden/intermediate widths do not imply an equal parameter or byte budget. The gate and up projections can be packed into one wider GEMM, while SiLU and multiplication can be fused. This is why a clean architecture diagram and a profiler trace can show different operation counts while computing the same model. See [GLU Variants Improve Transformer](https://arxiv.org/abs/2002.05202).
...
- Given equal \(H\) and intermediate width, why does a SwiGLU FFN have a different parameter and memory path from a ReLU FFN?
- Where does sinusoidal position enter the original Transformer, and where does RoPE act? Why is swapping them not a serving-time optimization?
- Draw pre-norm and post-norm residual paths, then place Q/K norm. Which operations normalize the residual stream versus attention heads?
- Why does GQA reduce state width but not remove the linear full-history decode scan?
- For full, local and data-dependent sparse attention, separate positions stored from positions read and name the selection/metadata cost.
