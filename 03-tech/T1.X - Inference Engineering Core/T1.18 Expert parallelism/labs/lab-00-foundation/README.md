# T1.18-L00 — Expert-routing histogram
...
How can balanced request counts create skewed expert token load?
...
1. Create `results/T1.18-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Run or simulate a top-k router over controlled token batches and preserve per-expert assignments.
...
- Independent conditions: uniform logits, biased logits and bursty class mix.
...
- Measure: tokens/expert, coefficient of variation, dropped/rerouted tokens and capacity.
...
Falsifier: **Skew disappears after counting tokens rather than requests.**
