# T1.35-L00 — MoE memory and routing ledger
...
How can total parameters be large while active compute and memory traffic differ per token?
...
1. Create `results/T1.35-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Inspect a supported small-MoE config and trace top-k expert assignments for deterministic inputs.
...
- Independent conditions: two workload classes and fixed top-k.
...
- Measure: total/active parameters, expert histogram, router confidence and memory.
...
Falsifier: **Active parameters are equated with resident weight memory.**
