# T1.32-L00 — Precision decomposition
...
Which bytes and operations use weight, activation and KV precision in each condition?
...
1. Create `results/T1.32-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Inventory actual checkpoint format, runtime kernels and KV dtype; compute memory deltas before benchmarking.
...
- Independent conditions: BF16 baseline and one genuinely supported quantized configuration.
...
- Measure: weight/KV memory, load logs, kernel support and correctness.
...
Falsifier: **A quantization label is accepted without effective dtype/kernel evidence.**
