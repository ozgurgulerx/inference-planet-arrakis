# T1.28-L01 — Direct versus staged path

Status: planned, not executed · Scope: core · Level: medium · Hardware: N2 GDR required · Dependencies: T1.28-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does GPUDirect remove host staging for the same payload and endpoint?
...
1. Create `results/T1.28-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Compare an explicit host-staged path with a documented direct GPU-buffer path using identical messages.
...
- Independent conditions: staged/direct × geometric sizes.
...
- Measure: latency, bandwidth, CPU, PCIe/NIC counters and trace.
...
Falsifier: **Lower CPU alone is accepted as proof that no bounce buffer exists.**
