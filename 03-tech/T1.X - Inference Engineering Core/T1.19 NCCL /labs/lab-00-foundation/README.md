# T1.19-L00 — NCCL communicator inventory
...
Which ranks, devices and links form this communicator?
...
1. Create `results/T1.19-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Record NCCL version, rank↔GPU mapping, topology output and a correctness-checked collective smoke test.
...
- Independent conditions: two local GPUs; one communicator; small payload.
...
- Measure: topology, initialization time, result checksum and debug log.
...
Falsifier: **The rank map is inferred rather than recorded.**
