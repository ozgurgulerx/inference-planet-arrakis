# T1.31-L02 — P:D ratio sensitivity

Status: planned, not executed · Scope: later · Level: expert · Hardware: G2+; extra GPUs conditional · Dependencies: T1.31-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which P:D ratio would meet a changed workload, and when would colocation win again?
...
1. Create `results/T1.31-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Model ratios from measured service demands; test only ratios possible on available GPUs and label the rest analytical.
...
- Independent conditions: prefill-heavy versus decode-heavy mix; at least two feasible ratios.
...
- Measure: per-pool utilization/queue, SLO goodput and GPU-seconds/task.
...
Falsifier: **The ratio recommendation ignores transfer or burst headroom.**
