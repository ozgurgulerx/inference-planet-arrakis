# T1.10-L02 — Clock/power/thermal boundary

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; controls may be read-only · Dependencies: T1.10-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Can a repeated kernel sustain its initial rate under the observed power and thermal envelope?
...
1. Create `results/T1.10-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Run a bounded steady workload while sampling clocks, power, temperature and throttle reasons.
...
- Independent conditions: cold start, warmed steady state and one allowed power condition.
...
- Measure: time series, steady throughput and clock-event reasons.
...
Falsifier: **A decline is called thermal without a matching temperature/clock event.**
