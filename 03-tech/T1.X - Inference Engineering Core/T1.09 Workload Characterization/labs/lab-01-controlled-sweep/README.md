# T1.09-L01 — Open versus closed loop

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.09-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How does client feedback hide overload in closed-loop tests?
...
1. Create `results/T1.09-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay one shape under fixed concurrency and independent offered-rate modes using the same server.
...
- Independent conditions: closed-loop concurrency 1/8; open-loop rates through saturation.
...
- Measure: offered/submitted/completed rate, lag, queue, latency and goodput.
...
Falsifier: **The open-loop generator misses schedule badly enough to invalidate offered load.**
