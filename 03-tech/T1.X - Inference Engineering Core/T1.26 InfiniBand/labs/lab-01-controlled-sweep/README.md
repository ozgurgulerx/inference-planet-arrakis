# T1.26-L01 — Point-to-point fabric curve

Status: planned, not executed · Scope: core · Level: medium · Hardware: N2 InfiniBand required · Dependencies: T1.26-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

What latency and bandwidth does this exact IB path deliver?
...
1. Create `results/T1.26-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run pinned `ib_write_bw`, `ib_read_bw` and a latency test with matching parameters at both endpoints.
...
- Independent conditions: geometric sizes; one then multiple queue depths.
...
- Measure: bandwidth, latency, CPU, retransmission/error indicators and commands.
...
Falsifier: **Client/server flags or units differ.**
