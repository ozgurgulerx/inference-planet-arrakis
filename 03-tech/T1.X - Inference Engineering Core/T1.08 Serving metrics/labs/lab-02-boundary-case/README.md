# T1.08-L02 — Goodput and percentile trap

Status: planned, not executed · Scope: later · Level: expert · Hardware: CPU using G1 artifacts · Dependencies: T1.08-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How do failures, mixed populations and percentile method change the reported conclusion?
...
1. Create `results/T1.08-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Analyze the same raw run with correct and deliberately flawed denominators/population mixes.
...
- Independent conditions: success-only versus all offered; pooled versus stratified shapes.
...
- Measure: goodput, rejection rate, p50/p95 with N, and decision reversal.
...
Falsifier: **The report cannot show which population each statistic describes.**
