# T1.43-L01 — One-variable intervention ladder

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.43-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does the smallest targeted intervention change the predicted metric and preserve correctness?
...
1. Create `results/T1.43-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Apply one bounded control based on the evidence ladder, then interleave baseline/intervention.
...
- Independent conditions: A/B/A at the same workload and warm state.
...
- Measure: target metric, collateral metrics, output validity and uncertainty.
...
Falsifier: **Multiple controls change or the target metric does not move as predicted.**
