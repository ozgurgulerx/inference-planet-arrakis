# T1.43-L02 — Blind diagnosis gauntlet

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; independent setup · Dependencies: T1.43-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Can the diagnosis survive an unseen fault and a counterfactual changed case?
...
1. Create `results/T1.43-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Have a separate operator select one safe fault from a fixed set; diagnose from artifacts, then test the rejected alternative.
...
- Independent conditions: hidden fault plus one confounder; hard safety bounds.
...
- Measure: ranked hypotheses, evidence requests, repair and retest.
...
Falsifier: **The fault label is revealed before the evidence plan or repair is accepted without retest.**
...
- [Primary release or dated documentation baseline](https://docs.nvidia.com/nsight-systems/UserGuide/)
