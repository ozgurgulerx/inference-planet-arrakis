# T1.04-L01 — Fragmentation and admission

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.04-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When does block granularity change which requests can be admitted?
...
1. Create `results/T1.04-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay a mixed-length request set in two orders and trace allocation, sharing, eviction and cleanup.
...
- Independent conditions: same multiset of lengths; short-first versus long-first; prefix cache off then on.
...
- Measure: admissions, free blocks, tail waste, preemptions and completion.
...
Falsifier: **Order changes latency but not the claimed allocation mechanism.**
