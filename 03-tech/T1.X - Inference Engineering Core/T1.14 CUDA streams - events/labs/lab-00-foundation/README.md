...
Which operations can overlap and which require an explicit dependency?
...
1. Create `results/T1.14-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Build H2D→kernel→D2H work in one and two streams using pinned memory and CUDA events.
...
- Independent conditions: single stream; two independent streams; two streams with dependency.
...
- Measure: event durations, wall time, timeline and checksum.
...
Falsifier: **Overlap is claimed without a timeline or the two streams operate on dependent data.**
