# T1.14-L02 — Overlap changed case

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 · Dependencies: T1.14-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does copy/compute overlap survive resource contention and smaller transfers?
...
1. Create `results/T1.14-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Sweep transfer size and kernel intensity with a dependency-correct double buffer.
...
- Independent conditions: small/large copies × light/heavy kernels.
...
- Measure: overlap ratio, throughput, timeline gaps and correctness.
...
Falsifier: **Apparent overlap does not improve critical-path time.**
