# T1.30-L01 — Async transfer size curve

Status: planned, not executed · Scope: core · Level: medium · Hardware: G2 local transfer; backend conditional · Dependencies: T1.30-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Where do setup, notification and data movement dominate a NIXL transfer?
...
1. Create `results/T1.30-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Measure enqueue, completion and total transfer over geometric sizes with reused and fresh registration.
...
- Independent conditions: size × fresh/reused region; bounded inflight depth.
...
- Measure: enqueue latency, completion latency, bandwidth, CPU and errors.
...
Falsifier: **Only enqueue time is reported as transfer time.**
