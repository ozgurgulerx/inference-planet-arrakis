...
Which CPU socket and memory node are local to each GPU and NIC?
...
1. Create `results/T1.23-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Capture `lscpu`, `numactl --hardware`, topology and device locality; mark hidden host data explicitly.
...
- Independent conditions: all exposed CPUs, GPUs and NICs.
...
- Measure: affinity graph and inaccessible fields.
...
Falsifier: **Container-visible CPUs are mistaken for physical socket topology.**
