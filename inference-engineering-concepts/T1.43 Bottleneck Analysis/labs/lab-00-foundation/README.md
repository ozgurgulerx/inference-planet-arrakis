# T1.43-L00 — Symptom-to-hypothesis map

Status: planned, not executed · Scope: core · Level: intro · Hardware: CPU using existing artifacts · Dependencies: none

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which competing mechanisms can produce the same latency or utilization symptom?
...
1. Create `results/T1.43-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Given one baseline run, write at least three mechanisms and a discriminating measurement for each before intervening.
...
- Independent conditions: one prefill-heavy and one decode-heavy symptom.
...
- Measure: hypothesis table, predictions and chosen next measurement.
...
Falsifier: **The proposed measurement cannot distinguish the mechanisms.**
