# T1.44-L02 — Burst/failure reserve

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; G2 for measured N−1 · Dependencies: T1.44-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How much headroom is needed for workload bursts or one lost replica?
...
1. Create `results/T1.44-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay a burst and model an N−1 condition from measured service demand; execute failure only on owned replicas.
...
- Independent conditions: steady, burst and N−1 changed cases.
...
- Measure: recovery, backlog, SLO goodput, reserve and cost.
...
Falsifier: **A steady-state average is used as the failure capacity plan.**
