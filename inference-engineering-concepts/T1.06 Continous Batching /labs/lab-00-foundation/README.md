# T1.06-L00 — Continuous-batch timeline
...
How does the running batch change at each scheduler step as requests arrive and finish?
...
1. Create `results/T1.06-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Capture per-step scheduled tokens, sequence membership and finish/release events for staggered requests.
...
- Independent conditions: four staggered requests with fixed token IDs and different OSL ceilings.
...
- Measure: batch membership movie, token budget, completions and idle gaps.
...
Falsifier: **The timeline cannot reconcile submitted work with emitted tokens and released state.**
