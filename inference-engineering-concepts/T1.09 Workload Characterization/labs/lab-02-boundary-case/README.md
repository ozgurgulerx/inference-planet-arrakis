# T1.09-L02 — Mixed-trace saturation

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 · Dependencies: T1.09-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Where is the sustainable knee for a representative ISL/OSL/prefix/tenant mix?
...
1. Create `results/T1.09-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay a deterministic mixed trace through warmup, rate sweep and recovery; freeze per-class SLOs first.
...
- Independent conditions: three rates around predicted knee plus a burst changed case.
...
- Measure: per-class SLO goodput, tails, cache hit, errors and recovery.
...
Falsifier: **The selected knee fails a held-out mix or burst without reserve.**
