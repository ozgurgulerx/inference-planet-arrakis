# T1.08-L01 — Client/server reconciliation

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.08-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which timing deltas can be compared without crossing clock domains?
...
1. Create `results/T1.08-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Correlate request IDs across client JSONL, engine logs and aggregate metrics; keep duration comparisons separate from timestamp subtraction.
...
- Independent conditions: isolated requests then two concurrent requests.
...
- Measure: client intervals, server intervals, queue time and attribution confidence.
...
Falsifier: **Aggregate histogram deltas are labeled per-request without isolation proof.**
