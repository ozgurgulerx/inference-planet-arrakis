# T1.22-L02 — Scale-up boundary

Status: planned, not executed · Scope: later · Level: expert · Hardware: G2; N2 extension blocked unless exposed · Dependencies: T1.22-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When does a fast intra-node fabric cease to matter because another resource is limiting?
...
1. Create `results/T1.22-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Combine one collective curve with controlled compute or a cross-node analytical bound.
...
- Independent conditions: small/large collective and one network-bound changed case.
...
- Measure: collective share, critical path and predicted cross-node ceiling.
...
Falsifier: **Cross-node conclusions are presented without network evidence.**
