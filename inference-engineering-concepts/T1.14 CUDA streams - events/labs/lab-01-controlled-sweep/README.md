# T1.14-L01 — Event timing audit

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.14-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When do CUDA events measure device work correctly and when does host timing mislead?
...
1. Create `results/T1.14-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Measure the same bounded sequence with events, synchronized wall time and unsynchronized wall time.
...
- Independent conditions: small/medium kernels; sync versus no-sync.
...
- Measure: three timing methods and ordering semantics.
...
Falsifier: **A method is accepted despite failing a known-delay calibration.**
