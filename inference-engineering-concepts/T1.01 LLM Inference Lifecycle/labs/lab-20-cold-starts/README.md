# T1.01-L20 — Three kinds of cold

**CORE · Medium · G1**
Dependencies: [L09](../lab-09-harness-validation/README.md), [L17](../lab-17-prefix-perturbation/README.md)
Execution status: proposed; GPU behavior unverified. Source check: 2026-09-20.

## Engineering question

Which cold state explains a slow first response?
...
1. **Prepare, 15–30 active minutes.** Separate fresh process/model readiness, new execution/compile shape and cold prefix in an otherwise warm process. Cache downloads independently from execution artifacts.
...
3. **Run, bounded session.** Use a factorial record of process/shape/prefix state; change one category at a time and retain startup time separately.
4. **Analyze and defend, 15–45 active minutes.** Cold-state timing matrix with restart/readiness events and cache identity.
...
Cold-state timing matrix with restart/readiness events and cache identity.
...
Diagnose a new-shape spike in an already warm server.

One slow request does not identify model loading, compilation or prefix work by itself.
