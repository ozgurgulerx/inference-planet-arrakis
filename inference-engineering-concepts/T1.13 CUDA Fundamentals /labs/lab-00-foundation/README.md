# T1.13-L00 — Threads, warps and correctness
...
How do indexing, bounds and synchronization determine CUDA kernel correctness?
...
1. Create `results/T1.13-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Build vector-add and tiled-reduction kernels using a supported CUDA path; include adversarial sizes and error checks.
...
- Independent conditions: aligned and non-multiple sizes; one intentionally faulty variant.
...
- Measure: checksum, CUDA errors, launch geometry and latency.
...
Falsifier: **A race appears only because host/device synchronization is missing from the test harness.**
