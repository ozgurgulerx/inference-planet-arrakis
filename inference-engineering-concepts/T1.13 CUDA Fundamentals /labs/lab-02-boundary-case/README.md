# T1.13-L02 — Synchronization failure case

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; sanitizer availability conditional · Dependencies: T1.13-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Can the test catch a stale read or cross-block synchronization assumption?
...
1. Create `results/T1.13-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Construct a bounded incorrect kernel, prove nondeterministic/wrong output, then repair it with a valid primitive.
...
- Independent conditions: increasing blocks and repeated seeds.
...
- Measure: failure frequency, sanitizer output if available and corrected checksum.
...
Falsifier: **The ‘failure’ is only a timing artifact in the host measurement.**
