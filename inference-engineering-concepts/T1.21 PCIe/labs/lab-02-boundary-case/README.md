# T1.21-L02 — Peer-to-peer matrix

Status: planned, not executed · Scope: later · Level: expert · Hardware: G2 · Dependencies: T1.21-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How do GPU pair and message size expose PCIe/P2P constraints?
...
1. Create `results/T1.21-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. On a multi-GPU Pod, verify peer access then measure every directed pair.
...
- Independent conditions: all GPU pairs × geometric sizes; P2P enabled/disabled where safe.
...
- Measure: bandwidth/latency matrix, topology and errors.
...
Falsifier: **P2P is claimed from device count without access/topology proof.**
