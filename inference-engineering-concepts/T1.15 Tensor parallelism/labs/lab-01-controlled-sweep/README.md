# T1.15-L01 — TP1 versus TP2 serving

Status: planned, not executed · Scope: core · Level: medium · Hardware: G2 topology verified · Dependencies: T1.15-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When does TP2 repay communication and duplicated/runtime overhead?
...
1. Create `results/T1.15-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Serve the same model/workload at TP1 and TP2 on one verified multi-GPU Pod.
...
- Independent conditions: ISL/OSL shapes short and long; concurrency 1/8; same precision.
...
- Measure: memory/rank, TTFT, TPOT, throughput, NCCL time and cost.
...
Falsifier: **TP2 comparison changes model/runtime limits or uses non-comparable GPUs.**
