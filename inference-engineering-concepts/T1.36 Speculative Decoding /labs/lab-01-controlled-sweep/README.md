# T1.36-L01 — Baseline versus speculative serving

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1; method/model support conditional · Dependencies: T1.36-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does a supported speculative method reduce target critical-path rounds without changing output validity?
...
1. Create `results/T1.36-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Run one version-supported speculative method against a non-speculative baseline with frozen sampling/quality. Record whether the method is MTP, a learned external drafter or another mechanism; never generalize its acceptance/cost result to the untested families.
...
- Independent conditions: short/long output × concurrency 1/8.
...
- Measure: acceptance, TTFT/TPOT, throughput, memory and task validity.
...
Falsifier: **Speedup excludes draft cost or compares different generation semantics.**
