# T1.16-L01 — Two-stage execution

Status: planned, not executed · Scope: core · Level: medium · Hardware: G2 · Dependencies: T1.16-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Does a real two-GPU pipeline follow the predicted microbatch frontier?
...
1. Create `results/T1.16-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Partition a small model or layer stack across two GPUs and record per-stage events.
...
- Independent conditions: microbatch sweep with fixed total batch; balanced then intentionally imbalanced split.
...
- Measure: timeline, stage idle, E2E and output equality.
...
Falsifier: **Outputs differ from the single-device reference or transfers are omitted.**
...
- [GPipe paper: pipeline bubbles and microbatches](https://arxiv.org/abs/1811.06965)
