# T1.25-L00 — Pinned and registered memory
...
What costs belong to allocation, pinning, registration and transfer?
...
1. Create `results/T1.25-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Measure pageable allocation, pinned allocation and reused pinned transfers separately.
...
- Independent conditions: small/large buffers; allocate each time versus reuse.
...
- Measure: allocation/registration proxy, transfer time, CPU and checksum.
...
Falsifier: **Pinned CUDA memory is assumed identical to NIC registration without evidence.**
