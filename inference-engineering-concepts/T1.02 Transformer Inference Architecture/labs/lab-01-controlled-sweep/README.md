# T1.02-L01 — Phase and context-depth matrix

Status: planned, not executed · Scope: core · Level: medium · Hardware: G1 · Dependencies: T1.02-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

Which layer operations and state terms make prefill and decode respond differently to context length?
...
1. Create `results/T1.02-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Instrument one model request and a phase microbenchmark with explicit NVTX ranges for projection, attention, MLP and cache update.
...
- Independent conditions: ISL 256/2048/8192/16384 × concurrency 1/4/8; fixed OSL; one variable per comparison.
...
- Measure: phase time, TTFT, TPOT, peak memory, kernel mix.
...
Falsifier: **Phase attribution does not predict the direction of the held-out context/concurrency result.**
