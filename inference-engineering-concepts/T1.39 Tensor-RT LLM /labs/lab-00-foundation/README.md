# T1.39-L00 — TensorRT-LLM compatibility and build
...
Can this exact model, GPU, precision and release produce a reproducible runnable engine?
...
1. Create `results/T1.39-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Use an official container/release, capture compatibility, build parameters and engine artifacts for a supported small model.
...
- Independent conditions: one conservative precision/profile.
...
- Measure: image digest, build log, engine metadata, smoke output and memory.
...
Falsifier: **An engine from a different GPU/release is reused without compatibility proof.**
