# T1.33-L02 — Throughput changed case

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; cross-model result observational · Dependencies: T1.33-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When does smaller attention state improve capacity but not single-request latency?
...
1. Create `results/T1.33-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Compare batch/concurrency regimes for two supported attention organizations.
...
- Independent conditions: concurrency 1 and near memory knee; matched task where defensible.
...
- Measure: capacity, TTFT/TPOT, token rate and quality caveat.
...
Falsifier: **Different models are treated as a clean architecture-only A/B.**
