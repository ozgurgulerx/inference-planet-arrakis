# T1.33-L00 — Attention-state estimator
...
How do MHA, GQA, MQA and MLA-like configurations change persistent state?
...
1. Create `results/T1.33-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Parse real model configs and derive heads, latent/state dimensions and bytes/token without forcing one universal formula.
...
- Independent conditions: at least one MHA and one GQA config; MLA separately labeled.
...
- Measure: parameter/state ledger and assumptions.
...
Falsifier: **MLA is reduced to KV-head count despite a different latent representation.**
