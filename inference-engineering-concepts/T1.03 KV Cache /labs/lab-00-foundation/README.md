# T1.03-L00 — KV growth and release
...
Does observed runtime state grow by the derived bytes per token and return to the allocator after request release?
...
1. Create `results/T1.03-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Extend the T1.01 KV arithmetic with a request-by-request trace of allocated blocks, used tokens and release events.
...
- Independent conditions: ISL 128/2048/6144; one request at a time; fixed OSL.
...
- Measure: predicted versus observed KV bytes, block rounding and post-request free capacity.
...
Falsifier: **Observed deltas cannot be explained within one block and allocator bookkeeping.**
