# T1.03-L02 — State-model contrast

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1; additional model conditional · Dependencies: T1.03-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

How must the capacity model change for MHA, GQA and a recurrent/hybrid state model?
...
1. Create `results/T1.03-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Derive each state equation from a real config and validate only on architectures supported by the pinned runtime.
...
- Independent conditions: matched token counts across two supported architectures; third may remain analytical.
...
- Measure: state bytes/token or bytes/sequence, allocation granularity and quality-valid throughput.
...
Falsifier: **A single KV formula is presented as universal despite a different state definition.**
