# T1.32-L02 — Kernel-support boundary

Status: planned, not executed · Scope: later · Level: expert · Hardware: G1 profiler permissions conditional · Dependencies: T1.32-L01

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When does lower precision fail to improve latency because conversion or unsupported shapes dominate?
...
1. Create `results/T1.32-L02/` and save the exact environment, command, version/digest and workload manifest there.
2. Choose a held-out shape/model path and inspect kernel traces around the suspected fallback.
...
- Independent conditions: supported versus fallback-prone shape.
...
- Measure: kernel names/dtypes, conversion time, latency and output validity.
...
Falsifier: **The fallback is inferred only from slower latency.**
