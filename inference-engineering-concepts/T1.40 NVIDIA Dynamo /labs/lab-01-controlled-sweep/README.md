# T1.40-L01 — Queue versus KV-aware routing

Status: planned, not executed · Scope: core · Level: medium · Hardware: CPU simulator then G2 deployment · Dependencies: T1.40-L00

Read the [environment and measurement contract](../environment.md) before using paid hardware.

## Engineering question

When should locality beat shortest-queue routing, and when should queue delay win?
...
1. Create `results/T1.40-L01/` and save the exact environment, command, version/digest and workload manifest there.
2. Replay exact-prefix and unique-prefix requests under two documented routing policies or a validated router simulator.
...
- Independent conditions: high/low reuse × balanced/skewed queues.
...
- Measure: route decisions, hit tokens, queue wait, TTFT and goodput.
...
Falsifier: **A simulator result is presented as deployed Dynamo evidence.**
