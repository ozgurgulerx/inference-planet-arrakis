...
What compute and bandwidth ceilings apply to one chosen inference operator?
...
1. Create `results/T1.12-L00/` and save the exact environment, command, version/digest and workload manifest there.
2. Derive FLOPs, compulsory bytes and arithmetic intensity from actual tensor shapes, then state ceiling assumptions.
...
- Independent conditions: one projection shape at batch/token dimensions 1, 8 and 64.
...
- Measure: FLOPs, bytes, intensity, roofline ceiling and assumptions.
...
Falsifier: **The ledger double-counts reuse or uses peak numbers for a different precision.**
